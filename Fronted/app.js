(function () {
  ...
  async function wireTripDetailsPage() {
    ...
    const resolveMapCoords = (cityName) => {
    ...
    const rawLocation = String(selectedTrip.location || location || '');
    const cityTokens = rawLocation.split(/[,&/|–\-]+/).map((w) => w.trim()).filter(Boolean);
    ...
    const foundInItinerary = allCityKeys
      .filter((k) => itineraryText.toLowerCase().includes(k))
  ...
  async function wireBookingPage() {
    ...
    // FIX: If we have an explicit tripId in URL or bookingMode is 'trip',
    // never treat this as a day/myTrip booking regardless of stale localStorage state.
    const isTripCatalogBooking = bookingMode === 'trip' || (!!queryTripId && bookingMode !== 'day');
    const selectedDayTotal = Number(candidateDayPayment?.total);
    const hasSelectedDayPayment =
      !isTripCatalogBooking &&
    ...
    const plannedLocationSummary = Array.from(
      ...
      .slice(0, 2)
      .join(' • ');
    // FIX: For trip-catalog bookings, always use the trip's own image & title.
  ...
  function setupBookingPaymentSelector() {
    // Map: method key → IDs of panels to show when this method is selected
    const METHOD_PANELS = {
      card:      ['card-inputs-section'],
      fawry:     ['fawry-instructions-section'],
      vodafone:  ['vodafone-inputs-section'],
      instapay:  ['instapay-instructions-section'],
      bank:      ['bank-instructions-section'],
      paypal:    ['paypal-instructions-section'],
    };

    // Collect all payment option elements (new HTML uses data-method)
    const allOptions = Array.from(document.querySelectorAll('[data-method]'));

    // Card input refs for validation
    ...
    // Default: first available method, or 'card'
    let selected = allOptions[0]?.dataset?.method || 'card';

    const applyState = () => {
      // Update each option's visual state
      allOptions.forEach((el) => {
        const method = el.dataset.method;
        const circle = el.querySelector('.pay-radio');
        if (method === selected) {
          el.classList.remove('border-outline-variant', 'border');
          el.classList.add('border-2', 'border-primary-container', 'bg-surface-container-low');
          if (circle) {
            circle.classList.remove('border-2', 'border-outline-variant');
            circle.classList.add('border-4', 'border-primary');
          }
        } else {
          el.classList.remove('border-2', 'border-primary-container', 'bg-surface-container-low');
          el.classList.add('border', 'border-outline-variant');
          if (circle) {
            circle.classList.remove('border-4', 'border-primary');
            circle.classList.add('border-2', 'border-outline-variant');
          }
        }
      });

      // Hide all panels first, then show only the selected one
      Object.values(METHOD_PANELS).flat().forEach((id) => {
        document.getElementById(id)?.classList.add('hidden');
      });
      (METHOD_PANELS[selected] || []).forEach((id) => {
